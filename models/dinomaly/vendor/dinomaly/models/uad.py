...
class ViTill(nn.Module):
    ...
    def forward(self, x):
        if hasattr(self.encoder, "forward_intermediates"):
            # timm DINOv3: RoPE needs the encoder's own block loop, and it
            # already strips the cls/storage tokens for us.
            with torch.no_grad():
                en_list = list(
                    self.encoder.forward_intermediates(
                        x,
                        indices=self.target_layers,
                        norm=False,
                        output_fmt="NLC",
            ...
            for i, blk in enumerate(self.encoder.blocks):
                ...
                if i in self.target_layers:
                    en_list.append(x)
            n_prefix = 1 + self.encoder.num_register_tokens
        side = int(math.sqrt(en_list[0].shape[1] - n_prefix))

        if self.remove_class_token:
            en_list = [e[:, n_prefix:, :] for e in en_list]
        ...
        if not self.remove_class_token:  # class tokens have not been removed above
            en = [e[:, n_prefix:, :] for e in en]
            de = [d[:, n_prefix:, :] for d in de]
