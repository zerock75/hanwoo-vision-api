...
_WEIGHTS_DIR = os.getenv("DINO_WEIGHTS_DIR", "backbones/weights")
...
# DINOv3 (patch 16) — served through timm, whose Eva ViT is the DINOv3
# architecture and whose weights are the Meta ones, un-gated.
_DINOV3_TIMM = {
...
def load_dinov3(arch):
    import timm

    if arch not in _DINOV3_TIMM:
        raise ValueError(f"DINOv3 arch must be one of {list(_DINOV3_TIMM)}, got {arch}")
    # pretrained=False: the Dinomaly checkpoint carries every encoder.* tensor and
    # load_state_dict overwrites these weights anyway, so downloading them would only
    # cost a HuggingFace round-trip on every cold start.
    return timm.create_model(_DINOV3_TIMM[arch], pretrained=False, num_classes=0)


def resolve_sizes(encoder_name, image_size=None, crop_size=None):
    """Input sizes for an encoder. The crop must divide into whole patches,
    or the token grid stops being square and the anomaly map can't be built."""
    patch = int(encoder_name.rsplit("_", 1)[-1])
    if crop_size is None:
        crop_size = 448 if patch == 16 else 392
        image_size = image_size or (512 if patch == 16 else 448)
    image_size = image_size or 448
    if crop_size % patch:
        crop_size -= crop_size % patch
        _logger.warning(
            "crop_size not a multiple of patch %d; using %d", patch, crop_size
        )
...
def load(name):
    ...
    if name.startswith("dinov3"):
        return load_dinov3(name.split("_")[-2])
        ...
        if "v2" in name:
            if "reg" in name:
                ...
                if arch == "base":
                    ...
                    raise ValueError(
                        "Invalid type of architecture. It must be either 'small' or 'base' or 'large."
                    )
            else:
                model = vision_transformer_dinov2.__dict__[f"vit_{arch}"](
                    patch_size=int(patchsize),
                ...
                if arch == "base":
                    ...
                    raise ValueError(
                        "Invalid type of architecture. It must be either 'small' or 'base'."
                    )
    ...
    if "mae" in name:
        ckpt_pth = download_cached_file(
            f"https://dl.fbaipublicfiles.com/mae/pretrain/mae_pretrain_vit_{arch}.pth"
        )
