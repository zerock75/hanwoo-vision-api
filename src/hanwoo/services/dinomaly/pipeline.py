...
LARGE_TARGET_LAYERS = [4, 6, 8, 10, 12, 14, 16, 18]
FUSE_LAYER_ENCODER = [[0, 1, 2, 3], [4, 5, 6, 7]]
FUSE_LAYER_DECODER = [[0, 1, 2, 3], [4, 5, 6, 7]]
N_DECODER_BLOCKS = 8
_SCORE_MODES = {"full", "roi_max", "roi_topk"}


def _build_transform(image_size: int, crop_size: int) -> transforms.Compose:
...
# (~600ms at 2560px), while the ROI mask is downsampled to the model crop anyway.
SEG_MAX_SIDE = 1024
...
class DinomalyService:
    def __init__(
        ...
        if score_mode not in _SCORE_MODES:
            raise ValueError(
                f"DINOMALY_SCORE_MODE must be one of {sorted(_SCORE_MODES)}, "
                f"got {score_mode!r}"
            )
        ...
        # Finalised in load() from the checkpoint; these are the fallback for
        # older checkpoints that carry no metadata.
        self.image_size, self.crop_size = vit_encoder.resolve_sizes(encoder_name)
        self.transform = _build_transform(self.image_size, self.crop_size)
    ...
    def load(self) -> None:
        ...
        # DINOv3 checkpoints describe their own geometry; the DINOv2 ones are a
        # bare state_dict, so fall back to the configured encoder name.
        meta = state if isinstance(state, dict) else {}
        self.encoder_name = meta.get("encoder", self.encoder_name)
        self.image_size, self.crop_size = vit_encoder.resolve_sizes(
            self.encoder_name, meta.get("image_size"), meta.get("crop_size")
        )
        self.transform = _build_transform(self.image_size, self.crop_size)

        encoder = vit_encoder.load(self.encoder_name)
        arch = self.encoder_name.split("_")[-2]
        target_layers = meta.get("target_layers", TARGET_LAYERS)
        if arch == "small":
            embed_dim, num_heads = 384, 6
        elif arch == "large":
            embed_dim, num_heads = 1024, 16
            # ViT-L is 24 blocks deep; layers 2-9 would all be low-level texture.
            target_layers = meta.get("target_layers", LARGE_TARGET_LAYERS)
        else:
        ...
        # strict=False is kept so a checkpoint may omit buffers, but an actual
        # key mismatch means the checkpoint does not belong to this backbone.
        # Left unchecked that produces a "healthy" service scoring garbage.
        incompatible = self.model.load_state_dict(state, strict=False)
        if incompatible.missing_keys or incompatible.unexpected_keys:
            raise RuntimeError(
                f"Checkpoint {self.model_path.name} does not match encoder "
                f"{self.encoder_name}: {len(incompatible.missing_keys)} missing, "
                f"{len(incompatible.unexpected_keys)} unexpected keys. "
                f"First missing: {incompatible.missing_keys[:3]}, "
                f"first unexpected: {incompatible.unexpected_keys[:3]}"
            )
        self.model.eval()
        logger.info(
            "Dinomaly loaded: %s | encoder=%s | %d->%d | mode=%s | threshold=%s",
            self.model_path.name, self.encoder_name,
            self.image_size, self.crop_size, self.score_mode, self.threshold,
        )
    ...
    def _prepare_meat_roi_masks(
        ...
        crop_start = (self.image_size - self.crop_size) // 2
        crop_end = self.image_size - crop_start
        ...
        if not np.any(model_inner_mask):
            model_inner_mask = np.ones((self.crop_size, self.crop_size), dtype=bool)
    ...
    def _compute_anomaly_map_and_score(
        ...
        t0 = time.perf_counter()
        anomaly_map = np.zeros((self.crop_size, self.crop_size), dtype=np.float32)
    ...
    def predict(self, image: Image.Image, return_heatmap: bool = True) -> dict:
        ...
        if return_heatmap:
            ...
            crop_start = (self.image_size - self.crop_size) // 2
            crop_end = self.image_size - crop_start
            x_start = int(crop_start * orig_w / self.image_size)
            x_end = int(crop_end * orig_w / self.image_size)
            y_start = int(crop_start * orig_h / self.image_size)
            y_end = int(crop_end * orig_h / self.image_size)
